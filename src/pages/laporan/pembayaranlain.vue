        ...
        <div class="main-card">
            ...
            <div class="toolbar">
                <div class="search-box">
                    ...
                    <input v-model="searchKeyword" placeholder="Cari nomor, keterangan..." class="search-input" />
            ...
            <div class="table-area">
                ...
                >
                    ...
                    <Column v-for="col in filterColumns" :key="col.field" :field="col.field" :sortable="true" :style="{ width: col.width, minWidth: col.minWidth || '60px', textAlign: col.align || 'left' }">
                      ...
                      <template v-else-if="col.field === 'Nilai'" #footer>
                    ...
                    <template #expansion="slotProps">
                        <div class="detail-content">
                            <div class="detail-header">
                                ...
                                <span>Detail Pembayaran - {{ slotProps.data.Nomor }}</span>
                            </div>
                            ...
                            <DataTable v-else :value="detailData[slotProps.data.Nomor] || []" class="detail-table" size="small">
                                <Column field="Kode" header="Kode Rek" style="width:120px" />
								<Column field="nama_rekening" header="Nama Rekening" style="min-width:250px" />
...
const filterColumns = [
    { field: "Nomor", header: "Nomor", width: "140px", minWidth: "120px" },
    { field: "Tanggal", header: "Tanggal", width: "100px", minWidth: "90px" },
    { field: "Keterangan", header: "Keterangan", width: "400px", minWidth: "250px" },
    { field: "Nilai", header: "Nilai", width: "150px", minWidth: "120px", align: "right", type: "currency" }
];
...
const loadData = async () => {
    ...
    try {
       ...
       const res = await $api.get("/v1/report/pembayaranlain", { params });
...
const onRowExpand = async (event: any) => {
    ...
    if (!detailData.value[nomor]) {
        ...
        try {
            const res = await $api.get(`/v1/report/pembayaranlain/${nomor}/detail`, { params: { start_date: formatDate(startDate.value), end_date: formatDate(endDate.value) } });
...
const loadAllDetails = async (): Promise<Record<string, any[]>> => {
    ...
    await Promise.all(filteredData.value.map(async row => {
        if (detailData.value[row.Nomor]) { all[row.Nomor] = detailData.value[row.Nomor]; return; }
        try { const res = await $api.get(`/v1/report/pembayaranlain/${row.Nomor}/detail`, { params: { start_date: formatDate(startDate.value), end_date: formatDate(endDate.value) } }); if (res.data.success) { all[row.Nomor] = res.data.data; detailData.value[row.Nomor] = res.data.data; } } catch (e) { console.error(e); }
    }));
    return all;
};
...
filteredData.value.forEach((r, i) => {
    const dr = ws1.getRow(i + 5);

    filterColumns.forEach((c, ci) => {
        const cell = dr.getCell(ci + 1);

        cell.value =
            c.type === "currency"
                ? (parseFloat(r[c.field]) || 0)
                : String(r[c.field] ?? "");

        cell.style = c.type === "currency" ? ns : ds;
    });
});

ws1.autoFilter = {
    from: "A4",
    to: `D${4 + filteredData.value.length}`
};
    ...
    filteredData.value.forEach(row => {
        ...
        if (details.length === 0) {
            ...
            details.forEach((d, di) => {
                ...
                dr.getCell(1).value = di === 0 ? row.Nomor : "";
                dr.getCell(2).value = d.Kode || "";
                dr.getCell(3).value = d.nama_rekening || "";
                dr.getCell(4).value = d.Keterangan || "";
                dr.getCell(5).value = parseFloat(d.Nilai) || 0;

                dr.getCell(5).style = ns;
...
a.href = url;
a.download =
    `Pembayaran_Lain_${formatDate(new Date())}` +
    `${exportWithDetail.value ? "_Lengkap" : ""}.xlsx`;
...
const doExportCSV = () => {
    ...
    const blob = new Blob([csv], { type: "text/csv;charset=utf-8;" }); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = `Pembayaranlain_${formatDate(new Date())}.csv`; a.click(); URL.revokeObjectURL(url);
    toast.add({ severity: "success", summary: "CSV", detail: "Berhasil", life: 2000 });
};
const doExportPDF = () => {

const period =
    `${startDate.value.toLocaleDateString("id-ID")} - ` +
...
if (exportWithDetail.value) {
    ...
    filteredData.value.forEach(r => {
        ...
        <table>
            <tr>
                ...
                <td>${r.Tanggal}</td>
                <td>${r.Keterangan}</td>
                <td class="r">${formatCurrency(r.Nilai)}</td>
            </tr>
        </table>
        `;

        const details = detailData.value[r.Nomor] || [];
        ...
        if (details.length > 0) {
            ...
            details.forEach(d => {
                ...
                <tr>
                    ...
                    <td>${d.Keterangan}</td>
                    <td class="r">${formatCurrency(d.Nilai)}</td>
                </tr>
                `;
            });
    ...
    <table>
        <thead>
            <tr>
                ${filterColumns.map(c => `<th>${c.header}</th>`).join("")}
            </tr>
        </thead>
        <tbody>
    `;
    ...
    filteredData.value.forEach(r => {
        ...
        <tr>
            ${filterColumns.map(c =>
                `<td class="${c.align === 'right' ? 'r' : ''}">
                    ${c.type === 'currency'
                        ? formatCurrency(r[c.field])
                        : (r[c.field] || '')}
                </td>`
            ).join("")}
        </tr>
...
.status-badge { display: inline-block; padding: 0.12rem 0.45rem; border-radius: 1rem; font-size: 0.68rem; font-weight: 600; &.status-sudah { background: #ecfdf5; color: #059669; } &.status-belum { background: #fffbeb; color: #d97706; } }
