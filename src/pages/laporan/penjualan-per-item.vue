        ...
        <div class="main-card">
            ...
            <div v-show="activeTab === 'pivot'" class="table-area">
                <div class="pivot-controls">
                    <div class="filter-item"><span class="filter-label">Baris</span><Select v-model="pivotRow" :options="pivotFields" optionLabel="label" optionValue="value" size="small" class="pivot-select" @change="buildPivot" /></div>
                    <div class="filter-item"><span class="filter-label">Kolom</span><Select v-model="pivotCol" :options="pivotFields" optionLabel="label" optionValue="value" size="small" class="pivot-select" @change="buildPivot" /></div>
                    <div class="filter-item"><span class="filter-label">Data</span><Select v-model="pivotData" :options="pivotDataFields" optionLabel="label" optionValue="value" size="small" class="pivot-select" @change="buildPivot" /></div>
                </div>
...
const pivotRow = ref('Bulan'); const pivotCol = ref('Tahun'); const pivotData = ref('Nilai')
...
const pivotDataFields = computed(() => { const f: any[] = [{ label: 'Qty', value: 'Qty' },{ label: 'Nilai', value: 'Nilai' },{ label: 'Disc', value: 'Disc' }]; if (isAdmin.value) f.push({ label: 'HPP', value: 'Hpp' },{ label: 'Margin', value: 'Margin' }); return f })
...
const gridColumns = computed(() => {
    const cols: any[] = [
        ...
        { field: 'Disc', header: 'Disc', width: '100px', minWidth: '85px', align: 'right' },
    ]
    if (isAdmin.value) { cols.push({ field: 'Hpp', header: 'HPP', width: '120px', minWidth: '100px', align: 'right' },{ field: 'Margin', header: 'Margin', width: '120px', minWidth: '100px', align: 'right' }) }
    return cols
})

const currencyFields = ['Nilai', 'Disc', 'Hpp', 'Margin']
const isCurrencyField = (f: string) => currencyFields.includes(f)
const isNumericField = (col: any) => col.field === 'Qty' || currencyFields.includes(col.field) || col.field === 'Tanggal'
...
const activeFiltersCount = computed(() => { let c = 0; Object.keys(activeColumnFilters.value).forEach(k => { if (activeColumnFilters.value[k]?.length > 0) c++; }); Object.keys(activeTextFilters.value).forEach(k => { if (activeTextFilters.value[k]?.trim()) c++; }); Object.keys(numericFilters.value).forEach(k => { if (numericFilters.value[k]) c++; }); return c; })

const evaluateCondition = (row: any, field: string, operator: string, val1: any, val2: any, isDate: boolean): boolean => {
    const rawVal = row[field]
    if (isDate) { const v = String(rawVal || ''); const v1 = String(val1 || ''); const v2 = String(val2 || ''); switch (operator) { case "eq": return v === v1; case "neq": return v !== v1; case "gt": return v > v1; case "gte": return v >= v1; case "lt": return v < v1; case "lte": return v <= v1; case "between": return v >= v1 && v <= v2; default: return true; } }
    else { const v = parseFloat(rawVal) || 0; const v1 = parseFloat(val1) || 0; const v2 = parseFloat(val2) || 0; switch (operator) { case "eq": return v === v1; case "neq": return v !== v1; case "gt": return v > v1; case "gte": return v >= v1; case "lt": return v < v1; case "lte": return v <= v1; case "between": return v >= v1 && v <= v2; default: return true; } }
}
...
const filteredData = computed(() => {
    ...
    Object.keys(numericFilters.value).forEach(field => { const filter = numericFilters.value[field]; if (!filter) return; const isDate = filter.type === 'date'; r = r.filter(row => { const cond1 = evaluateCondition(row, field, filter.operator1, filter.value1, filter.value1b, isDate); if (!filter.operator2 || (filter.value2 === null || filter.value2 === undefined || filter.value2 === '')) return cond1; const cond2 = evaluateCondition(row, field, filter.operator2, filter.value2, filter.value2b, isDate); return filter.logic === 'or' ? (cond1 || cond2) : (cond1 && cond2); }) })
    return r
})
...
const loadData = async () => { loading.value = true; try { const res = await $api.get('/v1/report/penjualan-per-item', { params: { start_date: formatDate(startDate.value), end_date: formatDate(endDate.value), is_admin: isAdmin.value } }); if (res.data.success) { data.value = res.data.data; buildFilterOptions(); buildPivot() } } catch (e: any) { toast.add({ severity: 'error', summary: 'Gagal', detail: e.response?.data?.message || 'Gagal', life: 3000 }) } finally { loading.value = false } }
const buildPivot = () => { const rows: Record<string, any> = {}; const colSet = new Set<string>(); let gt = 0; filteredData.value.forEach(item => { const rv = String(item[pivotRow.value] || ''); const cv = String(item[pivotCol.value] || ''); const dv = parseFloat(item[pivotData.value]) || 0; colSet.add(cv); if (!rows[rv]) rows[rv] = { label: rv, values: {}, total: 0 }; rows[rv].values[cv] = (rows[rv].values[cv] || 0) + dv; rows[rv].total += dv; gt += dv }); const cols = Array.from(colSet).sort((a, b) => { const na = parseInt(a), nb = parseInt(b); return isNaN(na) || isNaN(nb) ? a.localeCompare(b) : na - nb }); const colTotals: Record<string, number> = {}; cols.forEach(c => { colTotals[c] = Object.values(rows).reduce((s: number, r: any) => s + (r.values[c] || 0), 0) }); pivotResult.value = { columns: cols, rows: Object.values(rows), columnTotals: colTotals, grandTotal: gt } }
...
const buildFilterOptions = () => { gridColumns.value.forEach(col => { if (isNumericField(col) || col.field === 'Tanggal') return; const m = new Map<string, number>(); data.value.forEach(r => { const v = String(r[col.field] || ''); if (v) m.set(v, (m.get(v) || 0) + 1) }); filterOptionsCache.value[col.field] = Array.from(m.entries()).map(([v, c]) => ({ value: v, label: v, count: c })).sort((a, b) => a.label.localeCompare(b.label)) }) }
...
const doExportCSV = () => { let csv = '\uFEFF'; if (exportTarget.value === 'grid') { csv += gridColumns.value.map(c => `"${c.header}"`).join(',') + '\n'; filteredData.value.forEach(r => csv += gridColumns.value.map(c => `"${String(r[c.field] || '').replace(/"/g, '""')}"`).join(',') + '\n') } else { csv += `"${pivotRowLabel.value}","${pivotResult.value.columns.join('","')}","Total"\n`; pivotResult.value.rows.forEach(r => csv += `"${r.label}","${pivotResult.value.columns.map((c: string) => r.values[c] || 0).join('","')}","${r.total}"\n`) } const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `Penjualan_Per_Item_${formatDate(new Date())}.csv`; a.click(); URL.revokeObjectURL(url); toast.add({ severity: 'success', summary: 'CSV', detail: 'Berhasil', life: 2000 }) }
const doExportPDF = () => { const period = `${startDate.value.toLocaleDateString('id-ID')} - ${endDate.value.toLocaleDateString('id-ID')}`; let html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>body{font-family:Arial;padding:12px;font-size:8px}h1{font-size:13px;text-align:center;color:#059669}table{width:100%;border-collapse:collapse}th{background:#10b981;color:#fff;padding:3px;border:1px solid #059669;font-size:6px}td{padding:2px 3px;border:1px solid #e5e7eb;font-size:7px}.r{text-align:right}</style></head><body><h1>Penjualan Per Item</h1><p style="text-align:center;color:#6b7280">${period}</p>`; if (exportTarget.value === 'grid') { html += `<table><thead><tr>${gridColumns.value.map(c => `<th>${c.header}</th>`).join('')}</tr></thead><tbody>`; filteredData.value.forEach(r => html += `<tr>${gridColumns.value.map(c => `<td class="${c.align === 'right' ? 'r' : ''}">${isCurrencyField(c.field) ? formatCurrency(r[c.field]) : c.field === 'Qty' ? formatNumber(r[c.field]) : r[c.field] || ''}</td>`).join('')}</tr>`); html += `</tbody></table>` } else { html += `<table><thead><tr><th>${pivotRowLabel}</th>${pivotResult.value.columns.map(c => `<th>${c}</th>`).join('')}<th>Total</th></tr></thead><tbody>`; pivotResult.value.rows.forEach(r => html += `<tr><td>${r.label}</td>${pivotResult.value.columns.map(c => `<td class="r">${formatPivotValue(r.values[c])}</td>`).join('')}<td class="r"><strong>${formatPivotValue(r.total)}</strong></td></tr>`); html += `</tbody></table>` } html += `</body></html>`; const win = window.open('', '_blank', 'width=1000,height=700'); if (win) { win.document.write(html); win.document.close() } toast.add({ severity: 'success', summary: 'Print', detail: 'Gunakan Print > Save as PDF', life: 3000 }) }
const doExportExcel = async () => { const ExcelJS = await import('exceljs'); const wb = new ExcelJS.Workbook(); const hs: any = { font: { bold: true, color: { argb: 'FFFFFFFF' }, size: 10 }, fill: { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF10B981' } }, alignment: { horizontal: 'center', vertical: 'middle' } }; const ds: any = { font: { size: 9 } }; const ns: any = { ...ds, alignment: { horizontal: 'right' }, numFmt: '#,##0' }; if (exportTarget.value === 'grid') { const ws = wb.addWorksheet('Penjualan Per Item'); ws.getRow(4).height = 22; gridColumns.value.forEach((c, i) => { const cell = ws.getRow(4).getCell(i + 1); cell.value = c.header; cell.style = hs }); filteredData.value.forEach((r, i) => { const dr = ws.getRow(5 + i); gridColumns.value.forEach((c, ci) => { const cell = dr.getCell(ci + 1); const isNum = isCurrencyField(c.field) || c.field === 'Qty'; cell.value = isNum ? (parseFloat(r[c.field]) || 0) : String(r[c.field] ?? ''); cell.style = isNum ? ns : ds }) }); ws.autoFilter = { from: 'A4', to: `${String.fromCharCode(64 + gridColumns.value.length)}${4 + filteredData.value.length}` }; ws.views = [{ state: 'frozen', ySplit: 4 }] } else { const ws = wb.addWorksheet('Pivot'); ws.getRow(3).getCell(1).value = pivotRowLabel.value; ws.getRow(3).getCell(1).style = hs; pivotResult.value.columns.forEach((c, i) => { const cell = ws.getRow(3).getCell(i + 2); cell.value = String(c); cell.style = hs }); ws.getRow(3).getCell(pivotResult.value.columns.length + 2).value = 'Total'; ws.getRow(3).getCell(pivotResult.value.columns.length + 2).style = hs; pivotResult.value.rows.forEach((r, i) => { const dr = ws.getRow(4 + i); dr.getCell(1).value = r.label; dr.getCell(1).style = { ...ds, font: { bold: true } }; pivotResult.value.columns.forEach((c, ci) => { dr.getCell(ci + 2).value = r.values[c] || 0; dr.getCell(ci + 2).style = ns }); dr.getCell(pivotResult.value.columns.length + 2).value = r.total; dr.getCell(pivotResult.value.columns.length + 2).style = { ...ns, font: { bold: true } } }) } const buf = await wb.xlsx.writeBuffer(); const blob = new Blob([buf], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `Penjualan_Per_Item_${formatDate(new Date())}.xlsx`; a.click(); URL.revokeObjectURL(url); toast.add({ severity: 'success', summary: 'Excel', detail: 'Berhasil', life: 2000 }) }
