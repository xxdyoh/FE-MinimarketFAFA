        ...
        <template #footer>
          <div class="footer-summary-row">
            <span class="footer-value currency-text"
              >Rp Awal: {{ formatCurrency(totalRpAwal) }}</span
            >
            <span class="footer-value currency-text"
              >Rp Akhir: {{ formatCurrency(totalRpAkhir) }}</span
            >
          </div>
        </template>
      </DataTable>
        ...
        <div class="export-options">
          <div class="export-option active">
            ...
            <div class="option-text">
              <strong>Grid View</strong><small>Export data tabel</small>
            </div>
          </div>
        </div>
      </div>
...
const isNumericField = (col: any) =>
  col.type === "number" ||
  col.type === "currency" ||
  isCurrencyField(col.field);

const applyNumericFilter = (field: string, filter: any) => {
  if (filter) {
    numericFilters.value[field] = filter;
  } else {
    delete numericFilters.value[field];
  }
...
const filteredData = computed(() => {
  ...
  Object.keys(activeColumnFilters.value).forEach((f) => {
    ...
    if (v?.length > 0) r = r.filter((row) => v.includes(String(row[f])));
  });
...
const totalRpAwal = computed(() =>
  filteredData.value.reduce((s, r) => s + (parseFloat(r.Rp_Awal) || 0), 0)
);
const totalRpAkhir = computed(() =>
  filteredData.value.reduce((s, r) => s + (parseFloat(r.Rp_Akhir) || 0), 0)
);
...
const doExportCSV = () => {
  ...
  a.href = url;
  a.download = `Kartu_Stock_${formatDate(new Date())}.csv`;
  a.click();
  ...
  filteredData.value.forEach(
    (r) =>
      (html += `<tr>${gridColumns
        .map(
          (c) =>
            `<td class="${c.align === "right" ? "tr" : ""}">${
              isCurrencyField(c.field)
                ? formatCurrency(r[c.field])
                : c.type === "number"
                ? formatNumber(r[c.field])
                : r[c.field] || ""
            }</td>`
        )
        .join("")}</tr>`)
  );
  html += `</tbody></table><p style="text-align:right;font-size:8px;color:#9ca3af;margin-top:15px">Dicetak: ${new Date().toLocaleDateString(
    "id-ID"
  )}</p></body></html>`;
...
const doExportExcel = async () => {
  ...
  ws.mergeCells("A1:O1");
  ws.getCell("A1").value = "LAPORAN KARTU STOCK";
  ...
  exportData.forEach((r, i) => {
    ...
    gridColumns.forEach((c, ci) => {
      ...
      const isNum = isCurrencyField(c.field) || c.type === "number";
      cell.value = isNum
        ? parseFloat(r[c.field]) || 0
        : String(r[c.field] ?? "");
      cell.style = isNum ? ns : ds;
    });
  });
  ws.autoFilter = {
    from: { row: 4, column: 1 },
    to: { row: 4 + exportData.length, column: gridColumns.length },
  };
  ...
  const a = document.createElement("a");
  a.href = url;
  a.download = `Kartu_Stock_${formatDate(new Date())}.xlsx`;
  a.click();
  URL.revokeObjectURL(url);
  toast.add({
    severity: "success",
