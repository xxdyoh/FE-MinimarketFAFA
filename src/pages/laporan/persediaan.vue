  ...
  try {
    ...
    if (response.data.success) {
      ...
      toast.add({
        severity: "success",
        summary: "Berhasil",
        detail: `${data.value.length} data ditemukan`,
        life: 2000,
      });
    }
  } catch (error: any) {
    toast.add({
      severity: "error",
      summary: "Gagal",
      detail: error.response?.data?.message || "Gagal",
      life: 3000,
    });
  } finally {
    loading.value = false;
...
const onFilterPanelHide = (field: string) => {
  tempColumnFilters.value[field] = [
    ...(activeColumnFilters.value[field] || []),
  ];
};
...
const selectAll = (field: string) => {
  tempColumnFilters.value[field] = (filterOptionsCache.value[field] || []).map(
    (o) => o.value
  );
};
...
const hasColumnFilter = (field: string) =>
  activeColumnFilters.value[field]?.length > 0;
...
const exportCSV = () => {
  ...
  a.href = url;
  a.download = `Laporan_Persediaan_${
...
const exportExcel = async () => {
  ...
  worksheet.columns = filterColumns.map((col) => ({
    ...
    width:
      col.field === "Nama"
        ...
        : col.field === "Produsen"
        ? 20
        : col.field === "Kategori" || col.field === "Gudang"
        ? 18
        : col.field === "Nilai" || col.field === "HrgBeli"
        ? 18
        : 14,
  }));
  ...
  filteredData.value.forEach((row, index) => {
    ...
    dataRow.eachCell((cell, colNumber) => {
      ...
      if (col?.type === "currency" || col?.type === "number") {
        cell.style = (
          col.type === "currency" ? currencyStyle : numberStyle
        ) as any;
  ...
  toast.add({
    ...
    detail: "File Excel rapi berhasil didownload",
    life: 2000,
  });
};
...
const exportPDF = () => {
  ...
  const rows = filteredData.value.map((row) =>
    filterColumns.map((c) => row[c.field] || "")
  );
