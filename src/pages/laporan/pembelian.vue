    ...
    <div class="report-page">
        ...
        <Dialog v-model:visible="exportDialog" header="Export Laporan" :modal="true" :style="{ width: '420px' }">
            <div class="export-dialog-content">
                <p class="export-dialog-text">Pilih tipe export untuk laporan pembelian:</p>
                <div class="export-options">
                    <div class="export-option" :class="{ 'active': !exportWithDetail }" @click="exportWithDetail = false">
                        <div class="option-radio">
                            <div class="radio-circle" :class="{ 'checked': !exportWithDetail }"></div>
                        </div>
        ...
        <div class="report-card">
            ...
            <div class="date-filter-row">
                <div class="date-item">
                    ...
                    <DatePicker v-model="startDate" dateFormat="yy-mm-dd" showIcon size="small" @date-select="loadData" />
                </div>
                <div class="date-item">
                    <label>Tanggal Akhir</label>
                    <DatePicker v-model="endDate" dateFormat="yy-mm-dd" showIcon size="small" @date-select="loadData" />
                </div>
                <!-- <div class="date-item">
                    <label>Supplier</label>
                    <Select v-model="filterSupplier" :options="supplierList" optionLabel="nama" optionValue="kode" placeholder="Semua Supplier" size="small" showClear class="w-48" @change="loadData" />
                </div> -->
            </div>
            ...
            >
                ...
                <Column v-for="col in filterColumns" :key="col.field" :field="col.field" :sortable="true" :style="{ width: col.width || 'auto', textAlign: col.align || 'left' }">
                    <template #header>
                        <div class="column-header">
                            <span class="column-title">{{ col.header }}</span>
                            <Button 
                                icon="pi pi-filter" text rounded size="small"
                    ...
                    <Column field="Status_Bayar" header="Status" style="width: 90px; text-align: center">
    <template #body="{ data }">
        <span 
            class="status-badge"
            :class="data.Status_Bayar === 'Sudah' ? 'status-sudah' : 'status-belum'"
        >
            {{ data.Status_Bayar }}
        </span>
    </template>
                ...
                <template #expansion="slotProps">
                    <div class="detail-content">
                        ...
                        <DataTable v-else :value="detailData[slotProps.data.Nomor] || []" class="detail-table" size="small">
                            ...
                            <Column field="Nama" header="Nama Barang" style="min-width: 200px" />
                            <Column field="Satuan" header="Satuan" style="width: 70px; text-align: center" />
                            <Column field="Jumlah" header="Qty" style="width: 60px; text-align: center" />
                            <Column field="Harga" header="Harga" style="width: 110px; text-align: right">
                                <template #body="{ data }">{{ formatCurrency(data.Harga) }}</template>
                            </Column>
                            <Column field="Disc" header="Disc%" style="width: 60px; text-align: center" />
                            <Column field="Nilai" header="Nilai" style="width: 130px; text-align: right">
                                <template #body="{ data }"><span class="font-bold text-primary-600">{{ formatCurrency(data.Nilai) }}</span></template>
                            </Column>
                        </DataTable>
                    </div>
                </template>
...
const filterColumns = [
    ...
    { field: 'Total', header: 'Total', width: '130px', align: 'right', type: 'currency' },
    { field: 'Retur', header: 'Retur', width: '110px', align: 'right', type: 'currency' },
    { field: 'Bayar', header: 'Bayar', width: '110px', align: 'right', type: 'currency' },
...
const proceedExport = async () => {
    ...
    if (exportWithDetail.value) {
        toast.add({ severity: 'info', summary: 'Memproses', detail: 'Mengambil semua detail items...', life: 2000 })
...
const doExportExcel = async () => {
    ...
    filteredData.value.forEach((row, i) => {
        ...
        masterCols.forEach((col, idx) => {
            ...
            if (col.type === 'currency') {
                ...
                cell.style = {
                    ...dataStyle,
                    font: { ...dataStyle.font, color: { argb: val === 'Sudah' ? 'FF059669' : 'FFD97706' } },
    ...
    // Auto Filter
    ws1.autoFilter = { from: { row: 4, column: 1 }, to: { row: 4 + filteredData.value.length, column: 7 } }
    ...
    if (exportWithDetail.value) {
        ...
        // Header (Row 4)
        const detailHeaders = ['No. Nota', 'Supplier', 'Kode Brg', 'Nama Barang', 'Sat', 'Qty', 'Harga', 'Disc%', 'Nilai']
        ...
        detailHeaders.forEach((h, idx) => {
            ...
            cell.style = {
                ...headerStyle,
                fill: { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF3B82F6' } } // Biru untuk detail
            }
        })
                ...
                details.forEach((detail, detailIdx) => {
                    ...
                    for (let c = 1; c <= 9; c++) {
                        ...
                        if (detailIdx === 0) {
                            ...
                            dr.getCell(c).style = isNum ? numStyle : (isEven ? altRowStyle : dataStyle)
                        }
                    }
                    rowNum++
                })
        ...
        ws2.autoFilter = { from: { row: 4, column: 1 }, to: { row: rowNum - 1, column: 9 } }
    ...
    a.href = url
    a.download = `Laporan_Pembelian_${formatDate(new Date())}${exportWithDetail.value ? '_Lengkap' : ''}.xlsx`
    a.click()
...
const doExportCSV = () => {
    ...
    if (exportWithDetail.value) {
        csv += 'Nomor,Tanggal,Supplier,Total,Retur,Bayar,Status,Kode Barang,Nama Barang,Satuan,Qty,Harga,Disc%,Nilai\n'
...
<body>
    ...
    <div class="summary-cards">
        <div class="summary-card card-total">
            ...
            <div class="value">${formatCurrency(filteredData.value.reduce((s, r) => s + (parseFloat(r.Total) || 0), 0))}</div>
        </div>
        <div class="summary-card card-retur">
            <div class="label">Total Retur</div>
            <div class="value">${formatCurrency(filteredData.value.reduce((s, r) => s + (parseFloat(r.Retur) || 0), 0))}</div>
        </div>
        <div class="summary-card card-bayar">
            <div class="label">Total Bayar</div>
            <div class="value">${formatCurrency(filteredData.value.reduce((s, r) => s + (parseFloat(r.Bayar) || 0), 0))}</div>
        </div>
    </div>
`
    ...
    if (exportWithDetail.value) {
        ...
        filteredData.value.forEach((row, idx) => {
            const details = detailData.value[row.Nomor] || []
            const totalDetailNilai = details.reduce((s: number, d: any) => s + (parseFloat(d.Nilai) || 0), 0)
            ...
            <div class="nota-group">
                <div class="nota-header">
                    <div>
                        ...
                        <span style="margin-left: 10px; font-size: 10px; color: #6b7280;">| ${row.Tanggal}</span>
                    </div>
                    <div class="nota-info">
                        ${row.Supplier} | Total: ${formatCurrency(row.Total)} | Status: <span class="${row.Status_Bayar === 'Sudah' ? 'status-sudah' : 'status-belum'}">${row.Status_Bayar}</span>
                    </div>
                </div>`
                ...
                details.forEach((d: any) => {
                    html += `
                        <tr>
                            ...
                            <td class="text-right">${formatCurrency(d.Harga)}</td>
                            <td style="text-align:center">${d.Disc || 0}</td>
                            <td class="text-right">${formatCurrency(d.Nilai)}</td>
                        </tr>`
                })
                ...
                html += `
                    ...
                    <tfoot>
                        <tr style="background: #f3f4f6; font-weight: 600;">
                            ...
                            <td class="text-right" style="padding:5px 8px;">${formatCurrency(totalDetailNilai)}</td>
                        </tr>
                    </tfoot>
                </table>`
            } else {
        ...
        filteredData.value.forEach((row, i) => {
            html += `
                <tr>
                    ...
                    <td class="text-center"><span class="${row.Status_Bayar === 'Sudah' ? 'status-sudah' : 'status-belum'}">${row.Status_Bayar}</span></td>
                </tr>`
        })
        
        // Footer total
        const totalAll = filteredData.value.reduce((s, r) => s + (parseFloat(r.Total) || 0), 0)
        const returAll = filteredData.value.reduce((s, r) => s + (parseFloat(r.Retur) || 0), 0)
        const bayarAll = filteredData.value.reduce((s, r) => s + (parseFloat(r.Bayar) || 0), 0)
        ...
        html += `
            ...
            <tfoot>
                <tr style="background: #fef3c7; font-weight: 700;">
                    ...
                    <td class="text-right" style="padding:8px 10px;">${formatCurrency(totalAll)}</td>
                    <td class="text-right" style="padding:8px 10px;">${formatCurrency(returAll)}</td>
                    <td class="text-right" style="padding:8px 10px;">${formatCurrency(bayarAll)}</td>
                    <td></td>
                </tr>
            </tfoot>
        </table>`
    ...
    toast.add({ severity: 'success', summary: 'Preview PDF', detail: 'Klik tombol Print/Save di halaman', life: 3000 })
}
...
.filter-panel { width: 280px; max-height: 450px; display: flex; flex-direction: column; .filter-panel-header { display: flex; align-items: center; justify-content: space-between; padding: 0.75rem 1rem; border-bottom: 1px solid var(--surface-border); font-weight: 600; font-size: 0.875rem; } .filter-panel-search { padding: 0.75rem 1rem; border-bottom: 1px solid var(--surface-border); } .filter-panel-actions { display: flex; align-items: center; justify-content: space-between; padding: 0.5rem 1rem; border-bottom: 1px solid var(--surface-border); } .filter-panel-list { flex: 1; overflow-y: auto; max-height: 250px; padding: 0.5rem 0; .filter-option { display: flex; align-items: center; gap: 0.5rem; padding: 0.5rem 1rem; cursor: pointer; transition: background 0.2s; &:hover { background: var(--surface-50); } .filter-label { flex: 1; display: flex; align-items: center; justify-content: space-between; font-size: 0.813rem; cursor: pointer; margin: 0; .option-count { color: var(--text-color-secondary); font-size: 0.75rem; } } } .filter-empty { padding: 2rem 1rem; text-align: center; color: var(--text-color-secondary); font-size: 0.813rem; } } .filter-panel-footer { padding: 0.75rem 1rem; border-top: 1px solid var(--surface-border); } }

.detail-content { padding: 0.75rem 1rem; background: var(--surface-50); .detail-header { display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.5rem; font-weight: 600; font-size: 0.8rem; i { color: var(--primary-500); } } .detail-loading { display: flex; align-items: center; justify-content: center; gap: 0.5rem; padding: 1.5rem; color: var(--text-color-secondary); } }
.detail-table { background: var(--surface-card); border-radius: 0.375rem; overflow: hidden; border: 1px solid var(--surface-border); :deep(.p-datatable-tbody > tr > td) { padding: 0.3rem 0.5rem !important; font-size: 0.75rem; } }
