        ...
        <div class="main-card">
            ...
            <div class="toolbar">
                <div class="search-box">
                    ...
                    <input v-model="searchKeyword" placeholder="Cari nomor, supplier..." class="search-input" />
...
const filterColumns = [
    ...
    { field: "Supplier", header: "Supplier", width: "200px", minWidth: "150px" },
    { field: "Total", header: "Total", width: "130px", minWidth: "110px", align: "right", type: "currency" },
    { field: "Retur", header: "Retur", width: "110px", minWidth: "90px", align: "right", type: "currency" },
    { field: "Bayar", header: "Bayar", width: "110px", minWidth: "90px", align: "right", type: "currency" },
...
const totalRetur = computed(() => filteredData.value.reduce((s, r) => s + (parseFloat(r.Retur) || 0), 0));
const totalBayar = computed(() => filteredData.value.reduce((s, r) => s + (parseFloat(r.Bayar) || 0), 0));
...
const loadAllDetails = async (): Promise<Record<string, any[]>> => {
    ...
    await Promise.all(filteredData.value.map(async row => {
        ...
        try { const res = await $api.get(`/v1/report/pembelian/${row.Nomor}/detail`, { params: { start_date: formatDate(startDate.value), end_date: formatDate(endDate.value) } }); if (res.data.success) { all[row.Nomor] = res.data.data; detailData.value[row.Nomor] = res.data.data; } } catch (e) { console.error(e); }
    }));
    return all;
};
