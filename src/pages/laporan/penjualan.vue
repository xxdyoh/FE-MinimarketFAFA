...
const doExportCSV = () => {
    ...
    const blob=new Blob([csv],{type:'text/csv;charset=utf-8;'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=`Penjualan_${formatDate(new Date())}.csv`;a.click();URL.revokeObjectURL(url);
...
const doExportPDF = () => {
    ...
    else{html+=`<table><thead><tr>${gridColumns.map(c=>`<th>${c.header}</th>`).join('')}</tr></thead><tbody>`;filteredData.value.forEach(r=>html+=`<tr>${gridColumns.map(c=>`<td class="${c.align==='right'?'r':''}">${isCurrencyField(c.field)?formatCurrency(r[c.field]):r[c.field]||''}</td>`).join('')}</tr>`);html+=`</tbody></table>`}
    html+=`</body></html>`;
