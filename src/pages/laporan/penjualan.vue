                ...
                <template #expansion="slotProps">
                    <div class="detail-content">
                        ...
                        <DataTable v-else :value="detailData[slotProps.data.Nomor] || []" class="detail-table" size="small">
                            ...
                            <Column field="Harga" header="Harga" style="width: 110px; text-align: right"><template #body="{ data }">{{ formatCurrency(data.Harga) }}</template></Column>
                            ...
                            <Column field="Nilai" header="Nilai" style="width: 120px; text-align: right"><template #body="{ data }"><span class="font-bold text-primary-600">{{ formatCurrency(data.Nilai) }}</span></template></Column>
                        </DataTable>
                    </div>
                </template>
            </DataTable>
...
const filteredData = computed(() => {
    ...
    if (searchKeyword.value) {
        ...
        result = result.filter(r => Object.values(r).some(v => String(v).toLowerCase().includes(kw)))
    }
    Object.keys(activeColumnFilters.value).forEach(f => {
        const vals = activeColumnFilters.value[f]
        if (vals?.length > 0) result = result.filter(r => vals.includes(String(r[f])))
    })
    Object.keys(activeTextFilters.value).forEach(f => {
        const val = activeTextFilters.value[f]?.toLowerCase()
        if (val) result = result.filter(r => String(r[f] || '').toLowerCase().includes(val))
    })
    return result
})
...
const applyTextFilters = () => {
    ...
    filterableColumns.forEach(c => {
        if (textFilters.value[c.field]?.trim()) activeTextFilters.value[c.field] = textFilters.value[c.field].trim()
    })
}
...
const proceedExport = () => {
    ...
    toast.add({ severity: 'info', summary: 'Export', detail: 'Fitur export segera hadir', life: 2000 })
}
...
.export-dialog-content { .export-dialog-text { font-size: 0.9rem; color: var(--text-color-secondary); margin-bottom: 1.25rem; } .export-options { display: flex; flex-direction: column; gap: 0.75rem; } .export-option { display: flex; align-items: center; gap: 0.875rem; padding: 1rem 1.25rem; border: 2px solid var(--surface-border); border-radius: 0.75rem; cursor: pointer; transition: all 0.2s; .option-radio { flex-shrink: 0; .radio-circle { width: 1.25rem; height: 1.25rem; border-radius: 50%; border: 2px solid #cbd5e1; display: flex; align-items: center; justify-content: center; transition: all 0.2s; &.checked { border-color: var(--primary-500); background: var(--primary-500); position: relative; &::after { content: ''; width: 0.5rem; height: 0.5rem; border-radius: 50%; background: white; position: absolute; } } } } .option-icon { flex-shrink: 0; width: 2.5rem; height: 2.5rem; background: var(--surface-100); border-radius: 0.625rem; display: flex; align-items: center; justify-content: center; i { font-size: 1.25rem; color: var(--text-color-secondary); } } .option-text { flex: 1; strong { display: block; font-size: 0.9rem; color: var(--text-color); margin-bottom: 0.15rem; } small { font-size: 0.75rem; color: var(--text-color-secondary); } } &:hover { border-color: var(--primary-300); background: var(--primary-50); } &.active { border-color: var(--primary-500); background: var(--primary-50); .option-icon { background: var(--primary-100); i { color: var(--primary-600); } } .option-text strong { color: var(--primary-700); } } } }
@media (max-width: 768px) { .report-header { flex-direction: column; align-items: flex-start; gap: 1rem; .header-actions { width: 100%; button { flex: 1; } } } .browse-toolbar { flex-direction: column; gap: 0.75rem; .search-input { width: 100%; } } }
</style>